...
/// - This ensures that semantically equivalent strings (like composed vs
///   decomposed characters) are always encoded identically
///
...
///
/// dCBOR automatically ensures strings are in NFC form, which guarantees
/// consistent encoding across platforms. This is important for applications
/// that rely on deterministic encoding for hashing or signing operations.
///
/// For example, characters like "é" can be represented as either a single code
/// point (U+00E9, NFC) or as "e" followed by the combining acute accent (U+0065
/// U+0301, NFD). dCBOR ensures these are always encoded consistently in NFC
/// form.
...
impl From<String> for CBOR {
    fn from(value: String) -> Self { CBORCase::Text(value.clone()).into() }
}
