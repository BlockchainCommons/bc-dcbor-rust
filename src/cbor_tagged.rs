...
/// 
/// CBOR allows values to be "tagged" with semantic information using tag numbers.
/// The dCBOR library provides a set of traits for working with tagged values
/// in a type-safe manner.
/// 
/// Tags in CBOR provide additional context about how a value should be interpreted.
/// For example, tag 1 is used for dates, indicating the value is a timestamp.
///
/// This trait system allows Rust types to define their associated CBOR tags
/// and provide serialization/deserialization logic specifically for tagged values.
/// A trait for types that have an associated CBOR tag.
///
/// In CBOR, tags provide semantic information about how to interpret data items.
/// This trait defines which CBOR tag(s) are associated with a particular Rust type.
///
/// Implementing this trait is a prerequisite for implementing `CBORTaggedEncodable`
...
///
/// - **When encoding**: Only the first tag in the vector is used for serialization
/// - **When decoding**: Any of the tags in the vector will be accepted
///
/// This design solves several real-world problems:
///
/// 1. **IANA Registration Simplification**: If you initially choose a tag in the 
///    Specification Required range (24-32767) and later want to move to the simpler
///    First Come First Served range (32768+), you can migrate while maintaining 
///    compatibility with existing data.
///
/// 2. **Protocol Evolution**: As your protocol evolves, you can introduce new preferred
///    tags while still supporting data encoded with older tags.
///
/// 3. **Versioning**: Different tags can represent different versions of your data format
///    while sharing the same Rust type for handling.
///
/// ## Example: Single Tag
///
/// ```
...
pub trait CBORTagged {
    ...
    ///
    /// - The first tag in the vector is the "preferred" tag and will be used when encoding
    ///   values of this type via `CBORTaggedEncodable::tagged_cbor()`.
    ///
    ...
    /// (placed first in the vector) while still supporting older tags for decoding.
    ///
